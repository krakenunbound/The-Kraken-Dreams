...
class KrakenSuite:
    def __init__(self, root):
        ...
        self.monitoring_active = False # Flag for device monitoring
        self.setup_ui()
    ...
    def setup_styles(self):
        ...
        # IMPROVED VISIBILITY: Map states for better contrast
        self.style.map('TCombobox', 
                      fieldbackground=[('readonly', KRAKEN['bg_widget']), ('disabled', KRAKEN['bg_dark'])],
                      selectbackground=[('readonly', KRAKEN['accent'])],
                      selectforeground=[('readonly', KRAKEN['text_bright'])],
                      foreground=[('readonly', KRAKEN['text'])])
    ...
    def setup_record_tab(self):
        ...
        devices_frame, devices_content = self.create_section(container, "🎚️ AUDIO DEVICES")
        devices_frame.pack(fill=tk.X, pady=(0, 20))

        # Microphone
        mic_row = tk.Frame(devices_content, bg=KRAKEN['bg_mid'])
        ...
        # Bind events for persistence
        self.mic_combo.bind('<<ComboboxSelected>>', self.save_device_selection)
        self.system_combo.bind('<<ComboboxSelected>>', self.save_device_selection)

        refresh_btn = self.create_button(devices_content, "Refresh Devices", self.refresh_audio_devices)
        refresh_btn.pack(pady=(15, 5))

        # Output Section
        output_frame, output_content = self.create_section(container, "📁 OUTPUT")
        output_frame.pack(fill=tk.X, pady=(0, 20))

        folder_row = tk.Frame(output_content, bg=KRAKEN['bg_mid'])
        ...
        # Session Title
        title_row = tk.Frame(output_content, bg=KRAKEN['bg_mid'])
        title_row.pack(fill=tk.X, pady=5)
        tk.Label(title_row, text="Session Title:", font=('Segoe UI', 10),
                bg=KRAKEN['bg_mid'], fg=KRAKEN['text'], width=18, anchor='w').pack(side=tk.LEFT)
        self.session_title = tk.StringVar()
        title_entry = tk.Entry(title_row, textvariable=self.session_title, font=('Segoe UI', 10),
                               bg=KRAKEN['bg_widget'], fg=KRAKEN['text'], insertbackground=KRAKEN['text'],
                               relief='flat', bd=0)
        title_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(10, 10), ipady=5)
        tk.Label(title_row, text="(Optional)", font=('Segoe UI', 8),
                bg=KRAKEN['bg_mid'], fg=KRAKEN['text_dim']).pack(side=tk.RIGHT, padx=5)
        ...
        # Level meters
        meters_frame, meters_content = self.create_section(container, "📊 LEVELS")
        meters_frame.pack(fill=tk.X)

        mic_meter_row = tk.Frame(meters_content, bg=KRAKEN['bg_mid'])
    ...
    def setup_bard_tab(self):
        ...
        # Settings section
        settings_frame, settings_content = self.create_section(container, "⚙️ SETTINGS")
        settings_frame.pack(fill=tk.X, pady=(0, 15))

        # Provider selection
        provider_row = tk.Frame(settings_content, bg=KRAKEN['bg_mid'])
        ...
        party_frame, party_content = self.create_section(container, "🎭 THE PARTY")
        party_frame.pack(fill=tk.X, pady=(0, 10))

        self.party_avatars_frame = tk.Frame(party_content, bg=KRAKEN['bg_mid'])
    ...
    def refresh_audio_devices(self):
        # Stop monitoring if active to release devices for query
        ...
        # Note: We are currently excluding WASAPI Loopback (Output devices) because the installed 
        # sounddevice library version implies no loopback support in the python wrapper.
        # We will rely on "Stereo Mix" or other Input devices.

        # Mic dropdown: only input devices
        mic_device_list = [f"{i}: {name}" for i, name in input_devices]
        self.mic_combo['values'] = mic_device_list

        # System audio dropdown: inputs + outputs (labeled)
        system_device_list = [f"{i}: {name}" for i, name in input_devices]
        
        # Add output devices (marked) back in case user has a driver that supports direct input
        # or if we add loopback support in the future
        for i, name in output_devices:
            clean_name = name.split(" [")[0]
            if clean_name not in [n for _, n in input_devices]:
        ...
        if not mic_selected:
            for idx, (i, name) in enumerate(input_devices):
                # Prefer Yeti/USB mics over generic ones
                if ("Yeti" in name or "USB" in name) and "Stereo Mix" not in name:
                    self.mic_combo.current(idx)
                    mic_selected = True
                    break
        ...
        # Try to select saved system audio device
        system_selected = False
        if saved_system:
            # Clean up old saved names that might have [OUTPUT] in them
        ...
        # Fallback: Auto-select 'Stereo Mix' or 'Wave' or 'What U Hear'
        if not system_selected:
            for idx, (i, name) in enumerate(input_devices):
                if "Stereo Mix" in name or "What U Hear" in name or "Wave" in name:
                    ...
                    system_selected = True
                    break
            # If still not found, just pick the 2nd available input if possible (often system)
            if not system_selected and len(input_devices) > 1:
                self.system_combo.current(1)
            elif not system_selected and input_devices:
                self.system_combo.current(0)
                
        # Restart monitoring if it was active or if this is first load
        self.start_monitoring()
    ...
    def start_monitoring(self):
        ...
        if self.monitoring_active or self.is_recording:
            return
        ...
        threading.Thread(target=self.monitor_device, args=(mic_id, 1, self.mic_level), daemon=True).start()
        threading.Thread(target=self.monitor_device, args=(system_id, 2, self.system_level), daemon=True).start()
    ...
    def monitor_device(self, device_id, channels, level_bar):
        """Read audio device and update level meter only (no saving)."""
        try:
            # Try to open stream with requested channels, fallback to 1 if default fails
            # (Loopback logic removed as it requires specific sounddevice version/OS support causing issues)
            current_channels = channels
            stream = None
            
            try:
                stream = sd.InputStream(device=device_id, channels=current_channels, samplerate=self.sample_rate)
                stream.start()
            except Exception:
            ...
            if stream:
                with stream:
                    while self.monitoring_active:
                        if not self.root: break # Safety check
            ...
            # Log error safely one time
            if self.monitoring_active:
                self.monitoring_active = False
                msg = str(e)
                try:
                    self.root.after(0, lambda: self.log(f"Monitoring error on device {device_id}: {msg}"))
                except:
                    pass
            pass
    ...
    def save_device_selection(self, event=None):
        """Save current audio device selection to config."""
        mic_val = self.mic_combo.get()
        sys_val = self.system_combo.get()
        
        # Extract device names (remove ID prefix "0: ") to be robust against ID changes
        if mic_val and ": " in mic_val:
            self.config["mic_device"] = mic_val.split(": ", 1)[1]
        
        if sys_val and ": " in sys_val:
            if "[OUTPUT]" in sys_val:
                # Keep the [OUTPUT] tag for our logic
                 self.config["system_device"] = sys_val.split(": ", 1)[1]
            else:
                 self.config["system_device"] = sys_val.split(": ", 1)[1]
    ...
    def start_recording(self):
        self.stop_monitoring() # Stop monitoring before recording
        time.sleep(0.1)
        ...
        if mic_id is None or system_id is None:
            ...
            self.start_monitoring() # Restart monitoring
            return
    ...
    def record_device(self, device_id, data_list, channels, level_bar):
        try:
            # Simplified recording logic with fallback
            current_channels = channels
             # Try fallback to mono if stereo fails
            try:
                # Test stream open to check channel support
                s = sd.InputStream(device=device_id, channels=current_channels, samplerate=self.sample_rate)
                s.close()
            except:
                if current_channels > 1:
                    current_channels = 1

            with sd.InputStream(device=device_id, channels=current_channels, samplerate=self.sample_rate) as stream:
                while self.is_recording:
                    audio, _ = stream.read(1024)
                    
                    # If we fell back to mono but original data structure expected stereo, duplicate it?
                    # Or just append as is and let stop_recording handle it.
                    # The stop_recording function handles shape checking so this is fine.
            ...
            msg = str(e)
            try:
                self.root.after(0, lambda: self.log(f"Recording error: {msg}"))
            except:
                pass
    ...
    def stop_recording(self):
        ...
        # Determine filename from session title if provided
        session_name = self.session_title.get().strip()
        if session_name:
